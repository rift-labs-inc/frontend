...
const MiniStatusBar = ({ selectedVault }) => {
    ...
    const swappedAmount = selectedVault.initialBalance ? BigNumber.from(selectedVault.initialBalance).sub(withdrawnAmount).sub(trueUnreservedBalance).sub(reservedAmount) : BigNumber.from(0);
    ...
    return (
        <Flex width='120px' align='center' justify='flex-end' gap='12px'>
            ...
            <Flex width='60px' height='17px' borderRadius='10px' border={`1.5px solid ${colors.borderGrayLight}`} overflow='hidden'>
